...
# Copyright (c) 2024-2025, Daniel Ohayon.
#
# Modified from Flash Attention to add block-sparse attention support.
#
# IMPORTANT: This implementation has been modified to support ONLY block-sparse attention
# with per-head per-block thresholds. The CUDA kernels have been changed to implement
# block-sparse attention, and all functions now require a threshold tensor.
#
# Primary API:
#   - block_sparse_flash_attn_func(): Main function for block-sparse attention
#
# The threshold tensor should have shape (num_heads, max_blocks) where each element
# specifies the threshold for block selection at that head and query block position.
...
if USE_TRITON_ROCM:
    ...
    import block_sparse_flash_attn_2_cuda as flash_attn_gpu
...
# Block-sparse Flash Attention forward pass
# NOTE: The CUDA kernel has been modified to support block-sparse attention only.
# All attention functions in this file use per-head per-block thresholds.
...
def _flash_attn_forward(
    ...
    k: torch.Tensor,
    v: torch.Tensor,
    block_sparse_threshold: torch.Tensor,
    dropout_p: float,
    softmax_scale: float,
    causal: bool,
    ...
    """Block-sparse forward pass with per-head per-block threshold tensor.

    Args:
        q: Query tensor
        k: Key tensor
        v: Value tensor
        block_sparse_threshold: Tensor of shape (num_heads, max_blocks) with per-head per-block thresholds.
                               Blocks beyond max_blocks use the last threshold for that head.
        ...
        softcap: Softmax cap value (0.0 = disabled)
        alibi_slopes: ALiBi slopes tensor (optional)
        return_softmax: Whether to return softmax probabilities
    """
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    threshold_tensor = maybe_contiguous(block_sparse_threshold)

    out, softmax_lse, S_dmask, rng_state = flash_attn_gpu.fwd(
        q,
        k,
        v,
        threshold_tensor,  # arg3: Per-head per-block threshold tensor
        alibi_slopes,      # arg4: ALiBi slopes
        dropout_p,         # arg5: dropout
        softmax_scale,     # arg6: scale
        causal,            # arg7: causal
        window_size_left,  # arg8: window left
        window_size_right, # arg9: window right
        softcap,           # arg10: softcap
        -float('inf'),     # arg11: scalar threshold (unused when tensor is provided)
        return_softmax,    # arg12: return_softmax
        None,              # arg13: rng_generator
    )
...
class FlashAttnFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        # Block-sparse attention with per-head per-block tensor thresholds
        ...
        if is_grad:
            ...
            ctx.window_size = window_size
            ctx.softcap = softcap
            ctx.block_sparse_threshold = block_sparse_threshold
            ctx.alibi_slopes = alibi_slopes
            ctx.deterministic = deterministic
        out = out_padded[..., :head_size_og]
        return out if not return_softmax else (out, softmax_lse, S_dmask)
...
def block_sparse_flash_attn_func(
    q,
    k,
    v,
    block_sparse_threshold,  # Required: per-head per-block threshold tensor (nheads, max_blocks)
    dropout_p=0.0,
    softmax_scale=None,
    causal=False,
    window_size=(-1, -1),  # -1 means infinite context window
    ...
    deterministic=False,
    return_attn_probs=False,
    block_size=128,  # Block size for padding (default 128)
):
    """dropout_p should be set to 0.0 during evaluation
    Supports multi-query and grouped-query attention (MQA/GQA) by passing in KV with fewer heads
    than Q. Note that the number of heads in Q must be divisible by the number of heads in KV.
    ...
    Arguments:
        ...
        causal: bool. Whether to apply causal attention mask (e.g., for auto-regressive modeling).
        window_size: (left, right). If not (-1, -1), implements sliding window local attention.
        block_sparse_threshold: Tensor of shape (nheads, max_blocks). Per-head, per-block thresholds.
            Blocks beyond max_blocks use the last threshold for that head.
            Block i in head h uses threshold[h, min(i, max_blocks-1)].
            Each block's max attention score is compared to its threshold; blocks below threshold are skipped.
        alibi_slopes: (nheads,) or (batch_size, nheads), fp32. A bias of
            (-alibi_slope * |i + seqlen_k - seqlen_q - j|)
            is added to the attention score of query i and key j.
        deterministic: bool. Whether to use the deterministic implementation of the backward pass,
        ...
        return_attn_probs: bool. Whether to return the attention probabilities. This option is for
           testing only. The returned probabilities are not guaranteed to be correct
           (they might not have the right scaling).
        block_size: int. Block size for padding. Sequence length will be padded to a multiple of
            this value. Default is 128.
    Return:
        out: (batch_size, seqlen, nheads, headdim).
        softmax_lse [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen). The
            logsumexp of each row of the matrix QK^T * scaling (e.g., log of the softmax
        ...
        S_dmask [optional, if return_attn_probs=True]: (batch_size, nheads, seqlen, seqlen).
            ...
            pattern (negative means that location was dropped, nonnegative means it was kept).
    """
    # Pad sequence to block_size multiple if needed (only for causal attention)
    original_seq_len = None
    if causal and q.shape[1] % block_size != 0:
        seq_len = q.shape[1]
        padded_len = ((seq_len + block_size - 1) // block_size) * block_size
        pad_amount = padded_len - seq_len
        
        # Get the last token and repeat it for padding
        q_last = q[:, -1:, :, :]
        k_last = k[:, -1:, :, :]
        v_last = v[:, -1:, :, :]
        
        q_padding = q_last.repeat(1, pad_amount, 1, 1)
        k_padding = k_last.repeat(1, pad_amount, 1, 1)
        v_padding = v_last.repeat(1, pad_amount, 1, 1)
    ...
    # Truncate output if we padded
    if original_seq_len is not None:
        if return_attn_probs:
            out, softmax_lse, S_dmask = output
            out = out[:, :original_seq_len, :, :]
            softmax_lse = softmax_lse[:, :, :original_seq_len]
            S_dmask = S_dmask[:, :, :original_seq_len, :original_seq_len]
