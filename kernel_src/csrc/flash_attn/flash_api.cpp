...
void set_params_fprop(Flash_fwd_params &params,
                      ...
                      const float softcap,
                      const float block_sparse_threshold,
                      bool seqlenq_ngroups_swapped=false,
                      const bool unpadded_lse=false,
                      float* threshold_tensor_ptr=nullptr,
                      int threshold_max_blocks=0) {
    ...
    // Block sparse threshold - skip blocks where max(S * scale) < threshold
    params.block_sparse_threshold = block_sparse_threshold;

    // Handle threshold tensor if provided
    params.block_sparse_threshold_ptr = threshold_tensor_ptr;
    params.threshold_max_blocks = threshold_max_blocks;
    ...
    FP16_SWITCH(!params.is_bf16, [&] {
        HEADDIM_SWITCH(params.d, [&] {
            BOOL_SWITCH(params.is_causal, Is_causal, [&] {
                // Force row1 implementation always (never use splitkv)
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x round_multiple(head_size, 8)
    ...
    // Handle threshold tensor: Check if out_ contains a threshold tensor (2D float32)
    // vs an output tensor (4D half/bfloat16)
    float* threshold_tensor_ptr = nullptr;
    int threshold_max_blocks = 0;
    at::Tensor out;

    if (out_.has_value()) {
        auto out_or_threshold = out_.value();
        // Check if this is a threshold tensor (2D float32) or output tensor (4D)
        if (out_or_threshold.dim() == 2 && out_or_threshold.dtype() == torch::kFloat32) {
            // This is a threshold tensor: shape (nheads, max_blocks)
            CHECK_DEVICE(out_or_threshold);
            TORCH_CHECK(out_or_threshold.size(0) == sizes[2],  // original num_heads before swap
                       "Threshold tensor dim 0 must match num_heads, got ", out_or_threshold.size(0), " expected ", sizes[2]);
            TORCH_CHECK(out_or_threshold.is_contiguous(), "Threshold tensor must be contiguous");
            threshold_tensor_ptr = out_or_threshold.data_ptr<float>();
            threshold_max_blocks = out_or_threshold.size(1);
            // Create output tensor since out_ was used for threshold
            out = torch::empty_like(q);
        } else {
            // This is an output tensor
            out = out_or_threshold;
    ...
    set_params_fprop(params,
                     ...
                     /*unpadded_lse=*/false,
                     threshold_tensor_ptr,
                     threshold_max_blocks
                     );
