...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    // Allocate shared memory for block-wide max reduction (for block sparsity)
    // We need kNWarps floats for the reduction buffer
    float* smem_max_reduce = reinterpret_cast<float*>(smem_ + size(sQ) * sizeof(Element) + size(sK) * sizeof(Element) + size(sV) * sizeof(Element));

    // Get the threshold for this block
    // If threshold tensor is provided, index into it: threshold[head, min(m_block, max_blocks-1)]
    // Otherwise use the single threshold value
    float current_threshold = params.block_sparse_threshold;
    if (params.block_sparse_threshold_ptr != nullptr) {
        const int threshold_block_idx = min(m_block, params.threshold_max_blocks - 1);
        const int threshold_offset = bidh * params.threshold_max_blocks + threshold_block_idx;
        current_threshold = params.block_sparse_threshold_ptr[threshold_offset];
    }
    ...
    for (; n_block >= n_block_min; --n_block) {
        ...
        // BLOCK SPARSITY: Compute S = Q @ K^T first, then check threshold before loading V
        FLASH_NAMESPACE::gemm</*A_in_regs=*/Kernel_traits::Is_Q_in_regs>(
            acc_s, tSrQ, tSrK, tSsQ, tSsK, tiled_mma, smem_tiled_copy_Q, smem_tiled_copy_K,
            smem_thr_copy_Q, smem_thr_copy_K
        );
        ...
        // BLOCK SPARSITY: Find max(S * scale) across the entire block
        float thread_max = -INFINITY;
        #pragma unroll
        for (int i = 0; i < size(acc_s); ++i) {
            float val = acc_s(i) * params.scale_softmax;
            thread_max = max(thread_max, val);
        }

        // Block-wide reduction to find global max
        float block_max_score = FLASH_NAMESPACE::block_max_reduce<kNWarps>(thread_max, tidx, smem_max_reduce);


        // BLOCK SPARSITY: Skip this block if max score is below threshold
        if (block_max_score < current_threshold) {

            // Still need to prefetch next K block for pipelining
            if (n_block > n_block_min) {
                FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tKgK(_, _, _, n_block - 1), tKsK, tKVcKV, tKVpKV);
                cute::cp_async_fence();
            }
            continue;  // Skip to next K/V block
        }

        // If above threshold, proceed with normal Flash Attention
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        // BLOCK SPARSITY: Compute S = Q @ K^T first
        ...
        // Apply mask BEFORE threshold check (causal mask can make everything -inf)
        mask.template apply_mask<Is_causal, Is_even_MN>(
            acc_s, n_block * kBlockN, m_block * kBlockM + (tidx / 32) * 16 + (tidx % 32) / 4, kNWarps * 16
        );

        // If above threshold, load V and proceed with normal Flash Attention
        // Advance gV
        if (masking_step > 0) {
            if (block_table == nullptr) {
                tVgV.data() = tVgV.data() + (-int(kBlockN * params.v_row_stride));
