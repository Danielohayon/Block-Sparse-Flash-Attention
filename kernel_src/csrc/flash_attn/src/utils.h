...
// Block-wide max reduction across all threads
// Returns the maximum value to all threads in the block
template<int kNWarps>
__forceinline__ __device__ float block_max_reduce(float thread_max, int tidx, float* smem_red) {
    ...
    MaxOp<float> max_op;
    float warp_max = Allreduce<kWarpSize>::run(thread_max, max_op);

    // Step 2: Write warp results to shared memory (one value per warp)
    if (lane_id == 0) {
        smem_red[warp_id] = warp_max;
    }
    __syncthreads();

    // Step 3: Final reduction across warps (done by first warp)
    float block_max = -INFINITY;
    if (warp_id == 0) {
        float val = (lane_id < kNWarps) ? smem_red[lane_id] : -INFINITY;
        block_max = Allreduce<kWarpSize>::run(val, max_op);
    }

    // Step 4: Broadcast result to all threads
    if (tidx == 0) {
        smem_red[0] = block_max;
    }
    __syncthreads();
